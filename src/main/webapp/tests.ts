...
function createPlace(name: string) {
    var place = new Place(0, name);
    places.push(place);
    return place;
}

function createPersely(name:string, place: Place) {
...
function createBudget(year: number, month: number) {
    var b = new Budget(year, month);
    budgets.push(b);
    return b;
}

function createBudgetEntry(budget: Budget, budgeted_to: string, budgetedValue: number, planningType: PlanningType) {
...
function createTransaction(src: string, type: TransactionType, dst: string, value: number, year: number, month: number) {
...
QUnit.test("Spending", function (assert) {
    ...
    var budget = createBudget(2014, 7);
    var budgetEntryForExpense = createBudgetEntry(budget, "forrásPersely", 100, PlanningType.Expense);
    var budgetEntryForSave = createBudgetEntry(budget, "forrásPersely", 100, PlanningType.Save);
    ...
    createTransaction("kp", TransactionType.Spending, "forrásPersely", 100, 2014, 7);
    createTransaction("kp", TransactionType.Spending, "forrásPersely", 100, 2014, 7);
    createTransaction("kp", TransactionType.Spending, "forrásPersely", 100, 2014, 7);
    recalc();
    assert.equal(persely.currentValue(), 0);
...
QUnit.test("Spending and Flagging", function (assert) {
    ...
    createTransaction("kp", TransactionType.Spending, "forrásPersely", 100, 2014, 7);
    createTransaction("kp", TransactionType.Flagging, "forrásPersely", 100, 2014, 7);
    createTransaction("kp", TransactionType.Spending, "forrásPersely", 100, 2014, 7);
    recalc();
...
QUnit.test("Spending and Flagging in multiple month", function (assert) {
    ...
    var budgetEntryForExpense1 = createBudgetEntry(budget1, "forrásPersely", 100, PlanningType.Expense);
    var budgetEntryForSave1 = createBudgetEntry(budget1, "forrásPersely", 100, PlanningType.Save);
    var budget2 = createBudget(2014, 8);
    var budgetEntryForExpense2 = createBudgetEntry(budget2, "forrásPersely", 200, PlanningType.Expense);
    var budgetEntryForSave2 = createBudgetEntry(budget2, "forrásPersely", 200, PlanningType.Save);
    var budget3 = createBudget(2014, 9);
    var budgetEntryForExpense3 = createBudgetEntry(budget3, "forrásPersely", 300, PlanningType.Expense);
    var budgetEntryForSave3 = createBudgetEntry(budget3, "forrásPersely", 300, PlanningType.Save);
    ...
    createTransaction("kp", TransactionType.Spending, "forrásPersely", 100, 2014, 7);
    createTransaction("kp", TransactionType.Flagging, "forrásPersely", 150, 2014, 8);
    createTransaction("kp", TransactionType.Spending, "forrásPersely", 200, 2014, 9);
    recalc();
...
QUnit.test("Költés Budget nélkül szabályos", function (assert) {
    ...
    createTransaction("kp", TransactionType.Spending, "célPersely", 100, 2014, 7);
    recalc();
...
QUnit.test("Initializing place", function (assert) {
    ...
    createTransaction(null, TransactionType.IgnoreInBudgetFlagging, "kp", 100, 2014, 7);
    recalc();
...
QUnit.test("Initializing place and their Perselys", function (assert) {
    ...
    createTransaction("kp", TransactionType.IgnoreInBudgetFlagging, "placePersely", 100, 2014, 7);
    recalc();
    assert.equal(srcPlace.sumValue(), 100);
    assert.equal(srcPlace.myValue(), 0);
...
QUnit.test("Spending from Place", function (assert) {
    ...
    createTransaction(null, TransactionType.IgnoreInBudgetFlagging, "kp", 500, 2014, 7);
    createTransaction("kp", TransactionType.Spending, "célPersely", 100, 2014, 7);
    createTransaction("kp", TransactionType.Flagging, "célPersely", 100, 2014, 7);
    createTransaction("kp", TransactionType.Spending, "célPersely", 100, 2014, 7);
    recalc();
...
QUnit.test("Spending from A (which is a plance for B) to B", function (assert) {
    ...
    var budget = createBudget(2014, 7);
    var budgetEntryForExpense = createBudgetEntry(budget, "Nyaralás", 100, PlanningType.Expense);
    var budgetEntryForSave = createBudgetEntry(budget, "Nyaralás", 100, PlanningType.Save);
    ...
    createTransaction(null, TransactionType.IgnoreInBudgetFlagging, "kp", 200, 2014, 7);
    createTransaction("kp", TransactionType.Flagging, "Nyaralás", 100, 2014, 7);
    recalc();
    assert.equal(persely.currentValue(), 100);
    assert.equal(place.sumValue(), 200);
    ...
    createTransaction("kp", TransactionType.Spending, "Nyaralás", 100, 2014, 7);
    recalc();
    assert.equal(persely.currentValue(), 100);
...
QUnit.test("Spending from A to A", function (assert) {
    ...
    createTransaction("Nyaralás", TransactionType.Spending, "Nyaralás", 100, 2014, 7);
    recalc();
    assert.equal(persely.currentValue(), 0);
    assert.equal(place.sumValue(), 100, "200-ból 100at elköltöttünk, a másik százat csak átutaltuk a nyaralásra, de attól az még beleszámít a sumba");
    assert.equal(place.myValue(), 100);
...
QUnit.test("Balance honaprol honaóra", function (assert) {
    ...
    var budget = createBudget(2014, 7);
    var budgetEntryForExpense = createBudgetEntry(budget, "Nyaralás", 100, PlanningType.Expense);
    var budgetEntryForSave = createBudgetEntry(budget, "Nyaralás", 150, PlanningType.Save);

    var budget2 = createBudget(2014, 8);
    var budgetEntryForExpense2 = createBudgetEntry(budget2, "Nyaralás", 100, PlanningType.Expense);
    var budgetEntryForSave2 = createBudgetEntry(budget2, "Nyaralás", 100, PlanningType.Save);

    createTransaction(null, TransactionType.IgnoreInBudgetFlagging, "kp", 200, 2014, 7);
    createTransaction("kp", TransactionType.Flagging, "Nyaralás", 150, 2014, 7);
    createTransaction("Nyaralás", TransactionType.Spending, "Nyaralás", 50, 2014, 7);
...
QUnit.test("Balance(save) honaprol honaóra!", function (assert) {
    ...
    var budget2 = createBudget(2014, 8);
    var budgetEntryForSave2 = createBudgetEntry(budget2, "Nyaralás", 150, PlanningType.Save);

    createTransaction(null, TransactionType.IgnoreInBudgetFlagging, "kp", 500, 2014, 7);
    createTransaction("kp", TransactionType.Flagging, "Nyaralás", 150, 2014, 7);
    createTransaction("kp", TransactionType.Flagging, "Nyaralás", 150, 2014, 8);
...
QUnit.test("Gyűjtés esetén a táblázat balance-ába a persely összege kerül!", function (assert) {
    ...
    createTransaction(null, TransactionType.IgnoreInBudgetFlagging, "kp", 500, 2014, 7);
    createTransaction("kp", TransactionType.IgnoreInBudgetFlagging, "Nyaralás", 200, 2014, 7);
    createTransaction("kp", TransactionType.Flagging, "Nyaralás", 150, 2014, 7);
    createTransaction("kp", TransactionType.Flagging, "Nyaralás", 150, 2014, 8);
...
QUnit.test("Hónapokon átívelő információk", function (assert) {
    ...
    createTransaction(null, TransactionType.IgnoreInBudgetFlagging, "kp", 500, 2014, 7);
    createTransaction(null, TransactionType.Flagging, "kp", 1000, 2014, 7); // fizetés
    createTransaction("kp", TransactionType.Flagging, "forrásPersely", 100, 2014, 7);
    createTransaction("kp", TransactionType.Spending, "forrásPersely", 100, 2014, 7);

    createTransaction("kp", TransactionType.Spending, "forrásPersely", 2000, 2014, 8);
    recalc();
